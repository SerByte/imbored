...
/**
 * С какой уверенности семантике верят подбор и карточки.
 *
 * Выше потолка по одним тегам (TAGS_MAX_CONFIDENCE), то есть порог проходят
 * только игры, у которых оси уточнили отзывы: восьми разобранных отзывов уже
 * хватает (0.4 + 0.6 · 8/28 · 0.9 ≈ 0.55). Приор по тегам подбор и так
 * слышит — через сами теги (VIBE_TAGS, TIME_TAGS); повторить его осями
 * значило бы посчитать один и тот же голос дважды. Одно число на всех, кто
 * решает по семантике: строка сессии на /game, оси настроения в подборе,
 * время до веселья на карточке.
 */
export const SEMANTICS_MIN_CONFIDENCE = 0.5
