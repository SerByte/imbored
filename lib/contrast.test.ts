...
describe('контраст палитры', () => {
  ...
  describe('обвязка наследует зону, над которой стоит', () => {
    ...
    /*
     * Без `>`: подвал переехал внутрь обёртки плавной прокрутки, и прямым
     * потомком <body> быть перестал. Шапка осталась на месте — у неё
     * комбинатор сохранён.
     */
  ...
  /*
   * РОЛЬ НА ПОВЕРХНОСТИ, А НЕ НА ФОНЕ.
   *
   * Все проверки выше считают контраст относительно --bg. Но голый фон под
   * текстом — скорее исключение: продукт кладёт текст на две поверхности,
   * и обе СВЕТЛЕЕ фона в тёмной теме и ТЕМНЕЕ в светлой, то есть обе
   * съедают контраст.
   *
   *   --glass-bg — стекло панели: карточки, шапка, все крупные блоки;
   *   --surface  — подложка поля ввода и неактивной строки.
   *
   * Это ровно тот же класс ошибки, что уже разобран выше у акцента на
   * собственной плашке, только для нейтральных ролей — и он тоже был
   * настоящим: плейсхолдер главного поля ввода на лендинге («Ссылка на твой
   * Steam-профиль или ник», то есть единственная инструкция экрана) давал
   * 4.22:1 в тёмной теме и 4.19 в светлой при пороге 4.5. Токен считался на
   * фоне, а глаз видел поверхность.
   *
   * --track сюда НЕ входит намеренно: текста на нём нет нигде. Это заливка
   * незалитой части прогресса и неактивной точки шага — полоски высотой
   * 1–8 px и каркас страницы. Требовать от неё контраста с текстом значило
   * бы сторожить сочетание, которого не существует.
   */
  const SURFACES = ['--glass-bg', '--surface'] as const
  const TEXT_ROLES = ['--ink', '--dim', '--faint', '--ember-text', '--ok', '--info', '--danger']

  /** rgba(...) поверх непрозрачного фона — то, что реально под текстом. */
  function flatten(decl: string, bg: string): string {
    const m = decl.match(/rgba?\(([^)]+)\)/)
    if (!m) return decl
    ...
    return (
      '#' +
      [0, 1, 2]
        .map((i) => Math.round(n[i] * a + b[i] * (1 - a)))
        .map((v) => v.toString(16).padStart(2, '0'))
        .join('')
    )
  }
  ...
  for (const theme of THEMES) {
    test(`${theme.name}: текстовые роли читаются на поверхностях, а не только на фоне`, () => {
      const t = block(theme.selector)
      const bg = resolve(t, '--bg')
      for (const surface of SURFACES) {
        const under = flatten(resolve(t, surface), bg)
        for (const role of TEXT_ROLES) {
          expect(
            contrast(resolve(t, role), under),
            `${role} на ${surface} (${under}) в теме «${theme.name}»`,
          ).toBeGreaterThanOrEqual(AA)
        }
      }
    })
