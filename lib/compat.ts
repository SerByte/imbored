 ...
 *
 * ТОЛЬКО ДЛЯ ПАРЫ «ЧЕЛОВЕК — ЧЕЛОВЕК». Вычитание фона сделано под два профиля
 * из сотен тегов, и откат здесь — по паре: когда любая сторона взвешивается в
 * ноль, отдаётся сырой косинус. Для ранжирования списка это смешение шкал:
 * игра из одних частотных тегов (Singleplayer, Indie, Action) получила бы
 * сырые 0.6–0.9 против вычтенных 0.0x у настоящих соседей и встала бы в топ
 * почти каждой игры. Поэтому «человек — игра» и «игра — игра» (подбор, полки,
 * колода пати, будущие соседи игр) меряются одной мерой — weightedCosineTo из
 * lib/tagweight.ts: там откат решается один раз на всю сторону профиля, и
 * кандидат, взвешенный в ноль, получает ноль.
 */
