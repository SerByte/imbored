 ...
 * раз подряд. Защёлка на процесс гасила повторы, но первый вызов каждого
 * инстанса всё равно платил чтением на пути прогрева.
 *
 * Теперь пул досевается в migrateDb (lib/db.ts) под флагом в catalog_meta —
 * один раз на базу, а не на процесс, — и в seedDemo вместе с демо-карточками.
 * Модуль стал чистыми данными: lib/db импортирует его сам, и обратный импорт
 * замкнул бы их в цикл.
 */
