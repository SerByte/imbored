 ...
 *
 * Откат на сырой косинус — когда веса нет вовсе или одна из сторон взвешивается
 * в ноль (все её теги частотные или неизвестные карте). Тот же приём, что у
 * tasteCosine в lib/compat.ts: лучше старое поведение, чем молчаливые нули.
 */
export function weightedCosineTo(
  ...
  return (b) => {
    const wb = weighTags(b, w)
    return isEmpty(wb) ? cosine(a, b) : cosine(wa, wb)
  }
}
