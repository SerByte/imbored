...
import { pickQuizCovers, type QuizCovers } from '@/lib/quizart'
...
/**
 * Обложки под ответы квиза.
 *
 * Дешёвый маршрут по замыслу: снимок библиотеки и метаданные по её же appid —
 * два запроса, оба ограничены списком id. Ни каталожного пула, ни обновления
 * цен, ни Anthropic: экран не должен ждать, а квиз не должен стоить как выдача.
...
function json(covers: QuizCovers, counts: QuizCounts | null) {
  return NextResponse.json(
    // Поле отсутствует, а не равно нулю: клиент рисует блок только тогда, когда
    // числу можно верить — см. докблок MIN_COVERAGE в lib/quizcount.ts
    { covers, ...(counts ? { counts } : {}) },
    {
      headers: {
