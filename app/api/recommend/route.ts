 ...
 * ограничителя цикл «получить демо-сессию → запросить подборку» превращается в
 * чужой счёт за токены, и заметить это можно было бы только по биллингу.
 *
 * Считаем по обеим осям сразу. Один steamid — от человека, который завис на
 * кнопке «дальше»; один IP — от скрипта, который чеканит новые демо-личности
 * (они одноразовые, см. demoSteamId, и лимит по steamid сам по себе для него
 * не преграда).
 *
 * Отказ здесь — именно 429, а не тихий откат на эвристику. Эвристика
 * существует для СБОЯ модели, и подменять ею отказ по лимиту значит скрыть от
 * честного человека, что он упёрся в потолок.
 */
 ...
 *
 * Демо-выдача обязана быть настоящей: гость приходит с квиза ровно за тем,
