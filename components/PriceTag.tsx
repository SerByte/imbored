...
export function PriceTag({
  ...
  if (isFree || priceFinal === 0) {
    return <span className={`font-mono tabular-nums text-ember ${className}`}>бесплатно</span>
  }
  ...
  if (!discount) {
    return <span className={`font-mono tabular-nums text-ember ${className}`}>{price}</span>
  }
  ...
  return (
    <span className={`inline-flex items-baseline gap-2 ${className}`}>
      {showPercent && (
        <span
          className={`rounded-full bg-ember/15 text-ember font-mono font-semibold ${
      ...
      <span className="font-mono tabular-nums text-dim/60 line-through">
        {formatPrice(discount.initialCents)}
      </span>
      <span className={`font-mono tabular-nums text-ember ${hero ? 'text-base font-semibold' : ''}`}>
        {price}
      </span>
    </span>
  )
...
export function DiscountEnds({
  ...
  return <span className={`text-xs text-dim/70 ${className}`}>{discount.endsLabel}</span>
}
