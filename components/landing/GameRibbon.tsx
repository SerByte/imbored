...
/**
 * СКОЛЬКО РАЗНЫХ ОБЛОЖЕК БЕРЁТ ЛЕНТА НА УЗКОМ ЭКРАНЕ.
 *
 * Плиток в ленте много (замерено на 375×812 — сто тридцать шесть), но платим мы
 * не за плитки, а за РАЗНЫЕ картинки: повтор берётся из кэша и стоит ноль
 * байт. Разных там было двадцать восемь, и это 2.5 МБ с CDN Steam — впятеро
 * больше, чем весь остальной сайт вместе (539 КБ на своём домене).
 *
 * Продуктом пользуются вечером с телефона; два с половиной мегабайта
 ...
 *
 * На широком экране ограничение не нужно: там и полоса шире, и колонок больше,
 * и обложка видна крупнее — экономить нечем, а разнообразие там читается.
 */
const NARROW_COVERS = 12
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        /*
         * Пул обложек, а не весь список: см. NARROW_COVERS. Срез делается ОДИН
