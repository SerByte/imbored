 ...
 * базы доезжают.
 *
 * Правило про замеры. Цену и онлайн облако перемеряет само, на живых заходах
 * (refreshDeals, догрев в /api/prepare), а локальный каталог — снимок на момент
 * промоута. Замер едет, только если он не старше облачного, по своей отметке:
 * price_at у цены и скидки, ccu_at у онлайна (ПО_ОТМЕТКЕ в publishsql). Иначе
 * заливка воскрешала бы кончившиеся распродажи.
 *
