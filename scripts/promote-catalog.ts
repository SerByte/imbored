 ...
 *   2. одним батчем на 200 штук тянем теги с весами, категории, арт, кадры и
 *      трейлер;
...
async function main() {
  ...
  for (let i = 0; i < queue.length; i += STORE_BATCH) {
    ...
    // офлайн-скрипт, а игра, прошедшая промоут, сразу приходит в пул с
    // «20 секундами геймплея» — без ожидания крона карточек (lib/pagejob)
   ...
   *
   * GetItems не отдаёт ни жанров, ни медианы наигранного: их привозит
   * обогащение карточек (appdetails) прямо в базу. Кадры и трейлер он отдаёт,
   * но не у всех игр, и пустое апсерт не затирает (keepFilledSql). С заданным
